...
# FIXME wrap chords as
# <article itemscope class="chords">
# <header>
# <h1 itemprop="title">Song Title</h1>
# <h2 itemprop="artist">Song Artist</h2>
# </header>
# </article>

# FIXME p in a verse line should have "line" class
...
class TagsPreprocessor(Preprocessor):
    ...
    def run(self, lines):
        ...
        new_lines = []
        for line in lines:
            line = re.sub(r'(<![\w\d])#([a-zA-Z0-9]+\S+)', r'<span class="tag">\1</span>', line)
            new_lines.append(line)
        return new_lines
...
class ChordWrapperPreprocessor(Preprocessor):
    """
    Single line of widemd are detected and each chord is wrapped in span
    ...
    # FIXME add label to allow labeling diagrams
    # <label class="chord" for="F#m">F#m</label>
    # diagrams can be activated manually from any respective label
    def run(self, lines):
        new_lines = []
        for line in lines:
            if re.match(r'^ *(([A-H(*][1-9Mm#bdisue°+()/-]*) *)+ *$', line):
                line = re.sub(r'([A-H][1-9Mm#bdisue°+()/-]*)', r'<label class="chord" for="\1">\1</label>', line)
                line = re.sub(r'(\*+)', r'<span class="division">\1</span>', line)
            new_lines.append(line)
        return new_lines
...
class ChordsInlineProcessor(SimpleTagInlineProcessor):
    """
    Parse widemd and wrap inside tag
...
class ChordsSectionBlockProcessor(BlockProcessor):
    ...
    def run(self, parent: etree.Element, blocks: list[str]) -> bool | None:
        ...
        new_parent.set('class', 'widemd-section')
...
class ChordsMarkdownExtension(DefaultFileName, Extension):
    ...
    def extendMarkdown(self, md):
        ...
        md.parser.blockprocessors.register(
            ...
            'widemd-sections.fountain-brackets',
            200
        )
        ...
        #md.parser.blockprocessors.register(DotSectionBlockProcessor(md.parser), 'widemd-sections.fountain-dots', 175)
